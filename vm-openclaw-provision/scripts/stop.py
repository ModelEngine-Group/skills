...
def main():
    parser = argparse.ArgumentParser(description="Stop VM(s)")
    add_common_args(parser)

    parser.add_argument("--vm-id", "-v", required=True, help="VM ID to stop")
    ...
    print(f"Stopping VM {args.vm_id}...")

    try:
        task_id = client.stop_vm(site_id, args.vm_id)
        print(f"Stop task started, task_id: {task_id}")

        result = {"success": True, "vm_id": args.vm_id, "task_id": task_id}

        if args.wait:
            print(f"Waiting for VM to stop...")
            client.wait_for_task(site_id, task_id)
            result["stopped"] = True
            print(f"VM {args.vm_id} stopped successfully")
