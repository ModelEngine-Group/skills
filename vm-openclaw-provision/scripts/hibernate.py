...
def main():
    parser = argparse.ArgumentParser(description="Hibernate VM(s)")
    add_common_args(parser)

    parser.add_argument("--vm-id", "-v", required=True, help="VM ID to hibernate")
    ...
    print(f"Hibernating VM {args.vm_id}...")

    try:
        task_id = client.hibernate_vm(site_id, args.vm_id)
        print(f"Hibernate task started, task_id: {task_id}")

        result = {"success": True, "vm_id": args.vm_id, "task_id": task_id}

        if args.wait:
            print(f"Waiting for VM to hibernate...")
            client.wait_for_task(site_id, task_id)
            result["hibernated"] = True
            print(f"VM {args.vm_id} hibernated successfully")
