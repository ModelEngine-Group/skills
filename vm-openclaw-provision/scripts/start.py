...
def main():
    parser = argparse.ArgumentParser(description="Start VM(s)")
    add_common_args(parser)

    parser.add_argument("--vm-id", "-v", required=True, help="VM ID to start")
    ...
    print(f"Starting VM {args.vm_id}...")

    try:
        task_id = client.start_vm(site_id, args.vm_id)
        print(f"Start task started, task_id: {task_id}")

        result = {"success": True, "vm_id": args.vm_id, "task_id": task_id}

        if args.wait:
            print(f"Waiting for VM to start...")
            client.wait_for_task(site_id, task_id)
            result["started"] = True
            print(f"VM {args.vm_id} started successfully")
